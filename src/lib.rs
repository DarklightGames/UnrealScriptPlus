...
use crate::parser::{UnrealScriptParser};
...
pub enum ParsingError {
    ...
    PestError(pest_consume::Error<Rule>)
}
...
impl From<pest_consume::Error<Rule>> for ParsingError {
    fn from(error: pest_consume::Error<Rule>) -> Self {
...
fn parse_file(path: &str) -> Result<Program, ParsingError> {
...
fn unrealscriptplus(py: Python, m: &PyModule) -> PyResult<()> {
    ...
    fn pest_error_to_py_object(py: Python, e: pest_consume::Error<parser::UnrealScriptParser::Rule>) -> PyObject {
        let sequence = vec![
            (
